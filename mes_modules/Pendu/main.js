...
var chercheMot = function(nbLettres){
	var nbBoucles = 900000;
	do{
		motATrouver = util.valeurAleatoireDuTableau(ldm.dicoTab);
    nbBoucles--;
	}while (nbLettres != undefined && motATrouver.length != nbLettres && nbBoucles>0)
}
...
var getEtatDuJeu = function(message){
    ...
    res+="Nombre d'erreurs : "+nbErreurs+"/"+nbErreursPossibles+"\n";
...
var testeLettre = function(message){
  ...
  if (nbErreurs == nbErreursPossibles){
    message.channel.send(perdu(message));
		return 0;
  }
	etatDuJeu += getEtatDuJeu(message);
	message.channel.send(etatDuJeu);
...
var perdu = function (message){
  var reponse = "Perdu, tu réussiras peut-être la prochaine fois.\n";
